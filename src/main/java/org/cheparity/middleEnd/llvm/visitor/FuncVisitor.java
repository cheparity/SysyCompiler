...
public final class FuncVisitor implements ASTNodeVisitor {
    ...
    public void visit(ASTNode func) {
        ...
        if (paramOpt.isPresent()) {
            ...
            params.getChildren().stream().filter(node -> node.getGrammarType() == GrammarType.FUNC_FPARAM).forEach(node -> visitFuncParams(node, builder));
        }
        func.accept(new EntryBlockVisitor(function, builder));
    }
