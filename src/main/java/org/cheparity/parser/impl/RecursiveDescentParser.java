...
public class RecursiveDescentParser {
    ...
    private Optional<ASTNode> parseCompUnit() {
        ...
        this.nowSymbolTable = new SymbolTable(null); //Global symbol table
    ...
    private Optional<ASTNode> parseConstDecl() {
        ...
        semicolon.ifPresent(constDecl::addChild);
    ...
    private Optional<ASTNode> parseConstDef() {
        ...
        if (ident.isPresent()) {
            ...
            Symbol symbol = new Symbol(this.nowSymbolTable, SymbolType.VAL, ident.get().getToken());
            this.nowSymbolTable.addSymbol(symbol);
        } else {
            return failed(initIndex);
        }
    ...
    private Optional<ASTNode> parseVarDef() {
        ...
        if ((ident = parseTerminal(GrammarType.IDENTIFIER)).isPresent()) {
            ...
            Symbol symbol = new Symbol(this.nowSymbolTable, SymbolType.VAR, ident.get().getToken());
            this.nowSymbolTable.addSymbol(symbol);
        } else {
            return failed(initIndex);
        }
        ...
        while ((leftBracket = parseTerminal(GrammarType.LEFT_BRACKET)).isPresent()) {
            ...
            Optional<ASTLeaf> rightBracket = parseTerminal(GrammarType.RIGHT_BRACKET);
            rightBracket.ifPresent(varDef::addChild);
        }
    ...
    private Optional<ASTNode> parseFuncDef() {
        ...
        if (ident.isPresent()) {
            ...
            Symbol symbol = new Symbol(this.nowSymbolTable, SymbolType.FUNC, ident.get().getToken());
            this.nowSymbolTable.addSymbol(symbol);
        } else {
            return failed(initIndex);
        }
        ...
        rightParen.ifPresent(funcDef::addChild);
        Optional<ASTNode> block = parseBlock();
        if (block.isPresent()) {
            funcDef.addChild(block.get());
            this.nowSymbolTable = new SymbolTable(this.nowSymbolTable);
        } else {
            return failed(initIndex);
        }
        var symbol = new Symbol(this.nowSymbolTable, SymbolType.FUNC, ident.get().getToken());
        this.nowSymbolTable.addSymbol(symbol);
        return done(funcDef);
    }
     ...
     */
    private Optional<ASTNode> parseFuncFParams() {
        int initIndex = nowIndex;
     ...
     */
    private Optional<ASTNode> parseFuncFParam() {
        int initIndex = nowIndex;
        ...
        while ((leftBracket = parseTerminal(GrammarType.LEFT_BRACKET)).isPresent()) {
            ...
            while ((leftBracket = parseTerminal(GrammarType.LEFT_BRACKET)).isPresent()) {
                ...
                rightBracket.ifPresent(funcFParam::addChild);
            }
        }
        ...
        if (preRead4ParseStmt2JudgeLVal()) {
            ...
            if (lVal.isPresent()) {
                ...
                if (exp.isPresent()) {
                    ...
                    semicolon.ifPresent(stmt::addChild);
                    return done(stmt);
                }
                ...
                if (getintTk.isPresent()) {
                    ...
                    parseTerminal(GrammarType.RIGHT_PAREN).ifPresent(stmt::addChild);
                    parseTerminal(GrammarType.SEMICOLON).ifPresent(stmt::addChild);
                    return done(stmt);
                }
            }
        }
...
//        if (judgePreReadTerminal(0, GrammarType.IDENTIFIER) && judgePreReadTerminal(1, GrammarType.LEFT_PAREN)) {
    ...
    private Optional<ASTNode> parseLVal() {
        ...
        if (ident.isPresent()) {
            ...
            if (nowSymbolTable.lookup(ident.get().getToken().getRawValue()).isEmpty()) {
                var e = new UndefinedIdentException(ident.get().getToken());
     ...
     */
    private Optional<ASTNode> parseFuncRParams() {
        int initIndex = nowIndex;
...
//    private Optional<ASTLeaf> parseTerminal(GrammarType type, boolean checkError) {
//        Optional<ASTLeaf> res = parseTerminal(type);
//        if (!checkError || res.isPresent()) return res;
//        //handle error
//        var e = switch (type) {
//            case SEMICOLON -> new SemicolonMissedException(lastToken);
//            case RIGHT_PAREN -> new RParenMissedException(lastToken);
//            case RIGHT_BRACKET -> new RBracketMissedException(lastToken);
//            default -> new IllegalSymbolException(nowToken);
//        };
//        LOGGER.severe(e.getMessage());
//        error(e);
//        return Optional.empty();
